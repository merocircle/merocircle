...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isCreator) {
      ...
      return NextResponse.json({ channelUnreadCounts: {} });
    }

    const { data: channels, error: channelsError } = await channelsQuery;

    if (channelsError) {
      logger.error('Error fetching channels', 'CHANNEL_UNREAD_API', {
    ...
    // Get the last viewed timestamps from query params (optional)
    const { searchParams } = new URL(request.url);
    const lastViewedData = searchParams.get('lastViewed');
    let lastViewedMap: Record<string, Date> = {};
    
    if (lastViewedData) {
      try {
        lastViewedMap = JSON.parse(lastViewedData);
      } catch {
    ...
    // Query unread messages for each channel
    for (const channelId of channelIds) {
      ...
      if (countError) {
        logger.error('Error counting unread messages for channel', 'CHANNEL_UNREAD_API', {
        ...
        channelUnreadCounts[channelId] = count || 0;
      }
    }
    ...
    logger.error('Error in channel unread counts API', 'CHANNEL_UNREAD_API', {
